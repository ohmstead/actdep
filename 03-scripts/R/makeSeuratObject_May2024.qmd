...
meta$condition <- factor(meta$condition, levels = c('dSE', 'd30m', 'd6h', 'KA'))
...
meta <- meta |> 
  ...
  relocate(sublibrary, sample, condition, sex, .after = bc_wells) |>
  relocate(sample, condition, sex, .after = bc_wells) |> 
...
p1 <- DimPlot(nuclei, reduction = "umap", group.by = "condition") +
