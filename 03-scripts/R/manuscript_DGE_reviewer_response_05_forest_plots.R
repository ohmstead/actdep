...
#
# For the IEG + circadian gene panel (union of LoadGeneList("IEG") and
# LoadGeneList("circadian")), per subclass x contrast: log2FC point estimates
# with 95% CIs from DESeq2 (apeglm-shrunken, CI = estimate +/- 1.96*lfcSE),
# limma-voom, and the NB-GLMM with animal as a random effect. A second plot
# set adds a 4th series: DESeq2's raw (unshrunk) Wald estimate, i.e. before
# apeglm -- useful for seeing how much the shrinkage step itself moved the
# estimate, separate from cross-method disagreement.
#
# IEGs and circadian genes are split into separate facet panels (top/bottom)
# within each subclass x contrast plot rather than interleaved alphabetically,
# and methods are colored in grayscale (not by cmap) so the figure reproduces
# cleanly in print.
#
...
circadian_genes <- LoadGeneList("circadian")
panel_genes <- unique(c(ieg_genes, circadian_genes))

# Keeps IEGs and circadian genes visually separated within a single forest
# plot (facet_grid panels + strip labels) rather than interleaving them
# alphabetically.
gene_set_lookup <- tibble(
  gene = panel_genes,
  gene_set = if_else(gene %in% ieg_genes, "IEG", "Circadian")
)
gene_order <- c(sort(ieg_genes), sort(circadian_genes))

glmm_ci_ratio_max <- 10  # exclude fits where CI width >= this x |log2FoldChange|

df_glmm_all <- read_csv(file.path(out_dir, "glmm", "IEG-circadian_NB-GLMM_animal-random-effect.csv"),
...
method_colors_3 <- c("DESeq2 (apeglm)" = "black",
                     "limma-voom" = "gray55",
                     "NB-GLMM (animal RE)" = "gray75")

method_colors_4 <- c(method_colors_3, "DESeq2 (raw, unshrunk)" = "gray30")
...
plotForestSet <- function(df_forest, method_colors, pdf_path) {
  ...
  for (sc in unique(df_forest$subclass)) {
    ...
    for (ct in unique(df_sc$contrast)) {
      df_plot <- df_sc |>
        filter(contrast == ct) |>
        left_join(gene_set_lookup, by = "gene") |>
        mutate(gene = fct_rev(factor(gene, levels = gene_order)),
               gene_set = factor(gene_set, levels = c("IEG", "Circadian")))
