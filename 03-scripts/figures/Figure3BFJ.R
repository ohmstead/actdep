...
# load DEG csvs ----------------------------------------
csv_paths <- list(
  CA1 = "04-analysis/DEGs/Dec2024_activity_condition_pseudobulk/016_CA1-ProS_Glut__EE30m_vs_SE.csv",
  SST = "04-analysis/DEGs/Dec2024_activity_condition_pseudobulk/053_Sst_Gaba__EE30m_vs_SE.csv",
  Astrocyte = "04-analysis/DEGs/Dec2024_activity_condition_pseudobulk/319_Astro-TE_NN__EE30m_vs_SE.csv"
)
...
for (celltype in names(csv_results)) {
  ...
  p <- df |> ggplot() +
    ...
    labs(
      ...
      x = 'log2(Shrunken FC from SE)',
      y = '-log10(FDR)') +
    scale_x_continuous(limits = c(-1.5, 4), breaks = seq(4, -1.5, by = -1)) +
