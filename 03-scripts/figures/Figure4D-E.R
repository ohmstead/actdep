...
df_activity_plotting_IEGs <- df_active_cells_IEGs |> 
  ...
  mutate(supertype_label = paste0(
    'CA1 ',
    substr(supertype_name, 3, 4),
    '-',
    # get last character only
    substr(supertype_name, nchar(supertype_name), nchar(supertype_name))
  ))


# p_supertypes_IEG  ----------------------------------------
strip <- strip_themed(background_x = elem_list_rect(fill = 'gray80'))
...
p_supertypes_IEG <- ggplot(df_activity_plotting_IEGs) + 
  ...
  scale_x_discrete(labels = unique(sort(df_activity_plotting_IEGs$supertype_label))) +
  theme(
    legend.position = 'none',
    axis.title.x = element_blank(),
    axis.text.x = element_text(angle = 30, hjust = 1)
  ) +
  labs(y = 'Num. IEGs upregulated') +
