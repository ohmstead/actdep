...
p_activation_IEG <- ggplot(df_active_cells_IEGs, aes(x = condition, y = num_upregd_genes, fill = condition)) +
  geom_jitter(width = 0.3, height = 0.25, shape = 21, alpha = 0.5) +
...
lncRNA_degs <- read_csv("04-analysis/DEGs/condition/016_CA1-ProS_Glut_d30m_vs_dSE.csv") |> 
...
p_activation_lncRNA <- ggplot(df_active_cells_lncRNA, aes(x = condition, y = num_upregd_genes, fill = condition)) +
...
p_SE <- df_active_cells_IEGs |> 
  ...
  ggplot() +
  geom_density(aes(x = Npas4, fill = condition)) +
...
if (SAVE_PLOTS) {
  ...
  ggsave(plot = p_activation_IEG,
         path = '05-results/figure_1/raw_R_plots/', 
