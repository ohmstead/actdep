...
# animal random effect -- for 016 CA1-ProS Glut, EE6h vs. SE.
#
...
#
# Figure2_SuppFig4.R is the same analysis
# at the 30 min timepoint. Between them the two scripts produce the four
# panels the reviewer response needs, all on one gene set at matched N.
#
# WHICH GENES
#
# All four comparisons run on a single pinned gene set:
#   04-analysis/dge_method_comparison/rrho/016_CA1-ProS_Glut__analysis_genes_stratified.csv
...
# genes -- at EE30m, DESeq2-vs-voom concordance runs 0.997 in the top
# abundance decile but 0.955 in the bottom, so a top-N set overstates
# agreement (0.997 vs 0.986 genome-wide) while the stratified set pools to
# 0.985, essentially the genome-wide value. See
...
# RRHO grids and quadrant tables stay under 04-analysis; the assembled
# maps are a manuscript supplement and belong in 05-results.
fig_dir <- "05-results/Figure2_SuppFig5/raw_R_plots"
...
p_main <-
  plotRRHO(res_voom$map, "DESeq2 vs. limma-voom",
           x_label = "DESeq2 rank (down → up)", y_label = "limma-voom rank (down → up)", cap = shared_cap) +
  plotRRHO(res_glmm$map, "DESeq2 vs. NB-GLMM",
           x_label = "DESeq2 rank (down → up)", y_label = "NB-GLMM rank (down → up)", cap = shared_cap) +
  patchwork::plot_annotation(
    title = glue("{subclass}: {group1} vs {group2}")
  )
print(p_main)

p_controls <- res_voom$plot_scrambled + res_glmm$plot_scrambled +
  patchwork::plot_annotation(
...
if (exists("SAVE_PLOTS") && SAVE_PLOTS) {
  for (ext in c("png", "svg")) {
    ggsave(file.path(fig_dir, glue("RRHO_{group1}_vs_{group2}.{ext}")), p_main, width = 11, height = 5)
    ggsave(file.path(fig_dir, glue("RRHO_{group1}_vs_{group2}_negative_controls.{ext}")), p_controls, width = 11, height = 5)
  }
}
